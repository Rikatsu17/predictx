...
contract MarketFactory is Ownable {
    ...
    function createMarketCreate(string memory question, uint256 endTime) external returns (address) {
        PredictionMarket market = new PredictionMarket(outcomeShares, oracle, question, endTime);
        PredictAIOutcomeShares(outcomeShares).grantMarketRole(address(market));
