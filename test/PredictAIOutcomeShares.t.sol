...
contract PredictAIOutcomeSharesTest is Test {
    ...
    function setUp() public {
        ...
        shares.grantRole(shares.MARKET_ROLE(), market);
    }

    function testMintShares() public {
        vm.prank(market);

        shares.mint(alice, YES_TOKEN, 100);

        assertEq(shares.balanceOf(alice, YES_TOKEN), 100);
    }
    ...
    function testBurnShares() public {
        ...
        shares.burn(alice, YES_TOKEN, 50);

        vm.stopPrank();

        assertEq(shares.balanceOf(alice, YES_TOKEN), 50);
    }
